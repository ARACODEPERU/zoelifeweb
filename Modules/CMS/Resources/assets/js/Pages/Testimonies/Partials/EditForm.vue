...
const form = useForm({
    ...
    item_id: props.testimony.item_id,
...
const editarTestimony = () => {
    form.post(route('cms_testimonies_update'), {
        forceFormData: true,
        errorBag: 'editarTestimony',
        preserveScroll: true,
        onSuccess: () => {
            Swal2.fire({
                title: 'Enhorabuena',
...
<template>
    <FormSection @submitted="editarTestimony" class="">
        <template #title>
            Testimonio Detalles
        </template>

        <template #description>
            Crear editar Testimonio, Los campos con * son obligatorios
        </template>
        ...
        <template #form>
            ...
            <div class="col-span-6">
                <Checkbox v-model="form.status" label="Activo" />
            </div>
        </template>
